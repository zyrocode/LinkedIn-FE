            ...
            <div className="login-form mx-auto mt-5">
                <Container>
                  ...
                  <Form onSubmit={this.getCredentials}>
                    ...
                    <Input className="login-input"  type="email" placeholder="Email" value={this.state.email} onChange={e => this.setState({email: e.target.value})} />

                    <Input className="login-input"  type="password" placeholder="Password (6 or more characters)" value={this.state.password} onChange={e => this.setState({password: e.target.value})}/>

                    <Container><label className="pull-left checkbox-inline"><input type="checkbox" value={this.state.saveCredentials} onChange={e => this.setState({saveCredentials:!this.state.saveCredentials})}/> Keep me signed in</label></Container>
                    <Input className="btn btn-primary" type="submit" value="Log In" />
                    
                  </Form>
                  <p className="text-center"><small>Don't have an account yet? </small>
                  <Link to="/register" className="font-weight-bolder">Create an Account</Link>
                </p>

                <a href="http://app-be.azurewebsites.net/auth/facebook" className="btn btn-primary">Login With Facebook</a>
                </Container>
